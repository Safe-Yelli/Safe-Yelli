...
allData <- allData %>% 
  mutate(date = as.Date(`date and time`, '%d/%m/%Y'),
         ...
         hour = as.integer(format(as.POSIXct(`date and time`, tz="Mumbai"), 
                           format="%H"))
         )
```
...
allData$popupText <- 
  paste0('<strong>', allData$`kind of incident`, '</strong> <br/> <br/>',
         format(
           as.POSIXct(allData$`date and time`, format="%d-%m-%Y %H:%M", tz="Mumbai"),
