---
title: "Documenting harassment in Bengaluru"
...
allData$kindCopy <- allData$`kind of incident`
allData <- allData %>%  
  ...
  pivot_wider(id_cols = c(`description`,`date and time`, `lat`, `lng`, `kindCopy`),
              names_from = `kind of incident`, 
              values_from = `kind of incident`)
...
map <- leaflet(sharedD, height = "100vh") %>%
  ...
  fitBounds(77.528629,13.048286,77.659006,13.113414) %>%
           ...
           list(
             ...
             tags$div(class = "row",
                      tags$div(class = "col-md-12",
                               filter_select("Incident type", "Type", sharedD, group= ~`kindCopy`)
                               # filter_checkbox("Incident type",  sharedD, group= ~`Harassment`, inline=TRUE) ,
                               # filter_checkbox("Incident type",  sharedD, group= ~`Assault`, inline=TRUE) ,
                               # filter_checkbox("Incident type", "Type", sharedD, group= ~`Catcalling`, inline=TRUE) ,
                               # filter_checkbox("Incident type", "Type", sharedD, group= ~`Groping`, inline=TRUE) ,
                               # filter_checkbox("Incident type", "Type", sharedD, group= ~`I felt unsafe`, inline=TRUE) ,
                               # filter_checkbox("Incident type", "Type", sharedD, group= ~`I do not know what it is`, inline=TRUE) ,
                               # filter_checkbox("Incident type", "Type", sharedD, group= ~`Stalking`, inline=TRUE) ,
                               # filter_checkbox("Incident type", "Type", sharedD, group= ~` Theft`, inline=TRUE) 
                               ))
           )),
