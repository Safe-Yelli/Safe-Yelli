...
This is a collection of reports submitted to SafeYelli.in.
...
data <- data %>% 
  ...
  select( `date of incident`, `time of incident`, `description`, `kind of incident`, `location`) %>% 
...
tm_shape(bg) + 
  tm_rgb() + 
tm_shape(dataPlot) + 
  tm_dots(col = "kind of incident", size = 1) + 
  tm_scale_bar() + 
...
data %>% 
  select("date of incident", "time of incident", "description", "kind of incident", "lat", "lng") %>% 
  knitr::kable(
...
filePath = paste("../../static/collated-reports/", str_extract(knitr::current_input(), "[^.]+"),".csv") 
